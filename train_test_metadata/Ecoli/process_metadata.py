...
import numpy as np
import os
folder = '/home/ec2-user/ecoli_pasteur/'
pair_fn = f'{folder}/pairs_positive.txt'
pair_df = pd.read_csv(pair_fn, sep='\t', header=0, index_col=None)
pair_df.rename(columns = {'phage':'Phage_ID', 'bacterial_id':'Host_ID'}, inplace = True)
np.random.seed(0)
...
train_pair_df = pair_df[pair_df['train_or_test'] == 'train'][['Phage_ID', 'Host_ID', 'train_or_test', 'bacterial_host']]
test_pair_df = pair_df[pair_df['train_or_test'] == 'test'][['Phage_ID', 'Host_ID', 'train_or_test', 'bacterial_host']]
