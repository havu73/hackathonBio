...
import numpy as np
import os
folder = '/home/ec2-user/klebsiella/'
...
pair1_fn = f'{folder}/study1_pairs_positive.txt'
pair1_df = pd.read_csv(pair1_fn, sep='\t', header=0, index_col=None)
pair2_fn = f'{folder}/study2_pairs_positive.txt'
pair2_df = pd.read_csv(pair2_fn, sep='\t', header=0, index_col=None)
pair_df = pd.concat([pair1_df, pair2_df])
pair_df.rename(columns = {'phage':'Phage_ID', 'bacterial_host':'Host_ID'}, inplace = True)
